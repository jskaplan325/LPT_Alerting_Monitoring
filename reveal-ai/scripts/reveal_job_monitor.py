...
class RevealJobMonitor:
    ...
    def __init__(self, config: Dict, dry_run: bool = False, verbose: bool = False):
        ...
        if SCOM_AVAILABLE and self.config.get("scom_enabled", False):
            self.scom = SCOMIntegration(self.config, logging.getLogger(), "reveal_job", "reveal")
    ...
    def send_scom(self, result: Dict):
        ...
        try:
            check_result = {
                ...
                "failed_jobs": result.get("failed_jobs_count", 0),
                "stuck_jobs": result.get("stuck_jobs_count", 0),
