#!/usr/bin/env python3
"""
RelativityOne Alert Manager Monitor

Monitors the Alert Manager Agent - a single-instance agent responsible for:
- Native RelativityOne alerting
- System notifications
- Internal alert processing

This is "meta-monitoring" - monitoring the alerting system itself.
If the Alert Manager fails, native RelativityOne alerts stop working.
...
DEFAULT_CONFIG = {
    ...
    "state_file": "/tmp/alert_manager_state.json",
    "scom_enabled": False,
...
class AlertManagerMonitor:
    """Monitor the RelativityOne Alert Manager Agent."""
    ...
    def __init__(self, config: Dict, dry_run: bool = False, verbose: bool = False):
        ...
        if SCOM_AVAILABLE and self.config.get("scom_enabled", False):
            self.scom = SCOMIntegration(self.config, logging.getLogger(), "alert_manager", "relativity")
    ...
    def find_alert_manager_agent(self, agents: List[Dict]) -> Optional[Dict]:
        """Find the Alert Manager Agent from the list of agents."""
        search_term = self.config.get("agent_name_contains", "Alert").lower()

        # Priority search terms for Alert Manager specifically
        priority_terms = ["alert manager", "alertmanager", "alert agent"]

        # First try priority terms
        for agent in agents:
            agent_name = agent.get("Name", "").lower()
            for term in priority_terms:
                if term in agent_name:
                    return agent
        ...
        for agent in agents:
            ...
            if search_term in agent_name:
                # Avoid matching other agents with "alert" in name if possible
                return agent

        return None

    def check_agent_status(self, agent: Dict) -> Dict[str, Any]:
        """Check the status of the Alert Manager Agent and determine alert level."""
        result = {
        ...
        if not result["enabled"]:
            ...
            result["alert_message"] = "ALERT MANAGER DISABLED - Native RelativityOne alerting has stopped!"
            return result
        ...
        if status in ["not responding", "service not responding"]:
            result["level"] = "CRITICAL"
            result["alert_message"] = "ALERT MANAGER NOT RESPONDING - Platform alerting is down!"
            return result
        ...
        if result["last_activity"]:
            try:
                ...
                critical_threshold = self.config.get("activity_critical_minutes", 120)
                high_threshold = self.config.get("activity_high_minutes", 60)
                warning_threshold = self.config.get("activity_warning_minutes", 30)

                if age_minutes >= critical_threshold:
                    result["level"] = "CRITICAL"
                    result["alert_message"] = f"Alert Manager last activity was {round(age_minutes)} minutes ago (threshold: {critical_threshold} min)"
                elif age_minutes >= high_threshold:
                    result["level"] = "HIGH"
                    result["alert_message"] = f"Alert Manager last activity was {round(age_minutes)} minutes ago (threshold: {high_threshold} min)"
                elif age_minutes >= warning_threshold:
                    result["level"] = "WARNING"
                    result["alert_message"] = f"Alert Manager last activity was {round(age_minutes)} minutes ago (threshold: {warning_threshold} min)"

            except (ValueError, TypeError) as e:
                logging.warning(f"Could not parse last activity date: {e}")

        if result["level"] == "OK":
            result["alert_message"] = "Alert Manager is healthy - native alerting operational"

        return result

    def load_state(self) -> Dict:
        """Load previous state to prevent duplicate alerts."""
        state_file = self.config.get("state_file", "/tmp/alert_manager_state.json")
        try:
    ...
    def send_email(self, result: Dict):
        ...
        try:
            ...
            msg['Subject'] = f"[{result['level']}] RelativityOne Alert Manager Alert - META-MONITORING"

            body = f"""
RelativityOne Alert Manager Monitor Alert
*** THIS IS META-MONITORING - THE ALERTING SYSTEM IS DOWN ***
...
IMPACT: If the Alert Manager is down, native RelativityOne alerts
will NOT be sent. External monitoring (this script) is your only
notification source until the Alert Manager is restored.

Timestamp: {result['timestamp']}

---
This is an automated alert from the RelativityOne Alert Manager Monitor.
See RUNBOOK-006 for response procedures.
    ...
    def send_slack(self, result: Dict):
        ...
        if result["level"] != "OK":
            text = f"META-MONITORING: {text}\n_Native RelativityOne alerts may not be working!_"

        payload = {
            "attachments": [{
                "color": color,
                "title": f"Alert Manager Alert: {result['level']}",
                "text": text,
                "fields": [
                ...
                "footer": "RelativityOne Alert Manager Monitor (Meta-Monitoring)",
    ...
    def send_pagerduty(self, result: Dict):
        ...
        summary = result["alert_message"]
        if result["level"] != "OK":
            summary = f"META-MONITORING: {summary} - Native alerts may be down!"

        payload = {
            "routing_key": routing_key,
            "event_action": event_action,
            "dedup_key": "relativity-alert-manager",
            "payload": {
                "summary": summary,
                "source": "RelativityOne Alert Manager Monitor",
                "severity": severity,
                "custom_details": {
    ...
    def send_teams(self, result: Dict):
        ...
        message = result["alert_message"]
        if result["level"] != "OK":
            message = f"META-MONITORING: {message}"

        payload = {
            "@type": "MessageCard",
            "@context": "http://schema.org/extensions",
            "themeColor": color,
            "summary": f"Alert Manager Alert: {result['level']}",
            "sections": [{
                "activityTitle": f"Alert Manager Alert: {result['level']} (Meta-Monitoring)",
                "facts": [
    ...
    def run(self) -> int:
        ...
        logging.info("Starting Alert Manager monitoring check (meta-monitoring)...")
        ...
        try:
            ...
            # Find the Alert Manager Agent
            alert_manager = self.find_alert_manager_agent(agents)

            if not alert_manager:
                result = {
                    "level": "CRITICAL",
                    "alert_message": "ALERT MANAGER NOT FOUND - Agent may be deleted or renamed!",
                ...
                result = self.check_agent_status(alert_manager)

            # Log the result
            logging.info(f"Alert Manager Status: {result['level']}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Monitor RelativityOne Alert Manager Agent (Meta-Monitoring)"
    )
    parser.add_argument(
        "--config",
